# tests the FO hamiltonian along a MD simulation with fixed charge (NOM)

# failure of the test (if previous test were passed) can be caused by:
#   local or non-local electron phonon coupling (LIQM, LQM)
...
do
  ...
  if [ $run == "non-mpi" ]
    ...
    then mpirun -np 3 "$progpath/mdrun_mpi -pd -s ct.tpr -nsteps 20" > out.dat 2> /dev/null
  else 
    mpirun -np 1 "$progpath/mdrun_mpi -pd -s ct.tpr -nsteps 20" > out.dat 2> /dev/null
  fi
