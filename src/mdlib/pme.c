...
static int solve_pme_yzx_dftb(gmx_pme_t pme, t_complex *grid,
                         real ewaldcoeff, real vol)
{
    /* do recip sum over local cells in grid */
    /* y major, z middle, x minor or continuous */
    ...
    // iyz1 = local_ndata[YY]*local_ndata[ZZ]*(thread+1)/nthread;
    iyz0 = 0;
    iyz1 = local_ndata[YY]*local_ndata[ZZ];
...
static void gather_pot_bsplines_dftb(gmx_pme_t pme, real *grid,
                                   pme_atomcomm_t *atc, double *potential)
{
    splinedata_t *spline;
    int     n, ithx, ithy, ithz, i0, j0, k0;
    int     index_x, index_xy;
    int *   idxptr;
    real    pot, tx, ty, qn, gval;
    real    *thx, *thy, *thz;
    int     norder;
    int     order;
...
void make_bsplines_dftb(splinevec theta, splinevec dtheta, int order,
                   ...
                   /*  real charge[], gmx_bool bFreeEnergy == FALSE */
{
    /* construct splines for local atoms */
    int  i, ii;
    real *xptr;
...
static void
init_overlap_comm_dftb(pme_overlap_t *  ol,
                  int              norder,
//                  int              nnodes == 1
//                  int              nodeid == 0
                  int              ndata,
                      ...
                      int                 homenr)
{
     ...
     * but we do need the overlap in x because of the communication order.
     */
    init_overlap_comm_dftb(&pme->overlap[0], pme->pme_order,
                      pme->nkx,
                      (div_round_up(pme->nky, 1) + pme->pme_order)*(pme->nkz + pme->pme_order-1));

    /* Along overlap dim 1 we can send in multiple pulses in sum_fftgrid_dd.
     * We do this with an offset buffer of equal size, so we need to allocate
     * extra for the offset. That's what the (+1)*pme->nkz is for.
     */
    init_overlap_comm_dftb(&pme->overlap[1], pme->pme_order,
                      pme->nky,
                      (div_round_up(pme->nkx, 1) + pme->pme_order + 1)*pme->nkz);
     ...
     * The allocated size (pmegrid_n?) might be slightly larger.
     */
    pme->pmegrid_nx = pme->overlap[0].s2g1[0] - pme->overlap[0].s2g0[0];
    pme->pmegrid_ny = pme->overlap[1].s2g1[0] - pme->overlap[1].s2g0[0];
    pme->pmegrid_nz_base = pme->nkz;
    pme->pmegrid_nz      = pme->pmegrid_nz_base + pme->pme_order - 1;
    set_grid_alignment(&pme->pmegrid_nz, pme->pme_order);

    pme->pmegrid_start_ix = pme->overlap[0].s2g0[0];
    pme->pmegrid_start_iy = pme->overlap[1].s2g0[0];
    pme->pmegrid_start_iz = 0;
    ...
    pmegrids_init(&pme->pmegridA,
                  ...
                  pme->pme_order,
                  1,
                  pme->overlap[0].s2g1[0] - pme->overlap[0].s2g0[1],
                  pme->overlap[1].s2g1[0] - pme->overlap[1].s2g0[1]);
    ...
    /* Use atc[0] for spreading */
    init_atomcomm(pme, &pme->atc[0], NULL, 1, TRUE);
    ...
    /* Use fft5d, order after FFT is y major, z, x minor */
    snew(pme->work, pme->nthread);
    realloc_work(&pme->work[0], pme->nkx);
...
static void spread_on_grid_dftb(gmx_pme_t pme,
                           pme_atomcomm_t *atc, pmegrids_t *grids)
                           /* gmx_bool bCalcSplines == TRUE, gmx_bool bSpread == TRUE,
                           real *fftgrid)*/
{
        splinedata_t *spline;
        pmegrid_t *grid;
         ...
         * with help of some extra variables.
         */
        calc_interpolation_idx(pme, atc, 0, atc->n, 0);
...
int gmx_pme_do_dftb(gmx_pme_t pme,
               ...
               rvec x[],        /* rvec f[], */
               real *chargeA,   /* real *chargeB, */
               matrix box,      /* t_commrec *cr, int  maxshift_x, int maxshift_y, */
               t_nrnb *nrnb,    /* gmx_wallcycle_t wcycle, matrix vir,*/ 
               real ewaldcoeff,
               real *energy,    /* real lambda, real *dvdlambda, int flags */
               double *pot)
{
    int     d, i, j, ntot, npme;
    int     nx, ny, nz;
    ...
    /* Spread the charges on a grid */
    spread_on_grid_dftb(pme, &pme->atc[0], pmegrid);
    GMX_MPE_LOG(ev_spread_on_grid_finish);

    inc_nrnb(nrnb, eNR_WEIGHTS, DIM*atc->n);
    inc_nrnb(nrnb, eNR_SPREADQBSP, pme->pme_order*pme->pme_order*pme->pme_order*atc->n);
    ...
    gmx_parallel_3dfft_execute_dftb(pfft_setup, GMX_FFT_REAL_TO_COMPLEX,
                               fftgrid, cfftgrid);
    GMX_MPE_LOG(ev_gmxfft3d_finish);
    ...
    GMX_MPE_LOG(ev_solve_pme_start);
    loop_count =
        solve_pme_yzx_dftb(pme, cfftgrid, ewaldcoeff,
                      box[XX][XX]*box[YY][YY]*box[ZZ][ZZ]);
    GMX_MPE_LOG(ev_solve_pme_finish);
    inc_nrnb(nrnb, eNR_SOLVEPME, loop_count);
    ...
    gmx_parallel_3dfft_execute_dftb(pfft_setup, GMX_FFT_COMPLEX_TO_REAL,
                               cfftgrid, fftgrid);
    ...
    copy_fftgrid_to_pmegrid(pme, fftgrid, grid, /* pme->nthread == */ 1, /* thread == */ 0);

    /* calculate (not forces, but) potentials */
    unwrap_periodic_pmegrid(pme, grid);

    /* interpolate (not forces, but) potentials for our local atoms */
    GMX_BARRIER(cr->mpi_comm_mygroup);
    GMX_MPE_LOG(ev_gather_pot_bsplines_start);

    gather_pot_bsplines_dftb(pme, grid, atc, pot);

    GMX_MPE_LOG(ev_gather_pot_bsplines_finish);
