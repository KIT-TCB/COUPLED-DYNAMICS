...
void fft5d_execute_dftb(fft5d_plan plan) /* int thread, fft5d_time times */
{
    t_complex  *lin   = plan->lin;
    t_complex  *lout  = plan->lout;
    t_complex  *lout2 = plan->lout2;
    ...
    *C              = plan->C, *P = plan->P, **iNin = plan->iNin, **oNin = plan->oNin, **iNout = plan->iNout, **oNout = plan->oNout;
...

    {
        ...
        if (plan->nthreads == 1) /* should be always the case */
        {
            fftout = lout;
        }
        else
        ...
        if ((plan->flags&FFT5D_REALCOMPLEX) && !(plan->flags&FFT5D_BACKWARD) && s == 0)
        {
            gmx_fft_many_1d_real(p1d[s][0], (plan->flags&FFT5D_BACKWARD) ? GMX_FFT_COMPLEX_TO_REAL : GMX_FFT_REAL_TO_COMPLEX, lin+tstart, fftout+tstart);
        }
        else
        {
            gmx_fft_many_1d(     p1d[s][0], (plan->flags&FFT5D_BACKWARD) ? GMX_FFT_BACKWARD : GMX_FFT_FORWARD,               lin+tstart, fftout+tstart);
    ...
    if ((plan->flags&FFT5D_REALCOMPLEX) && (plan->flags&FFT5D_BACKWARD))
    {
        gmx_fft_many_1d_real(p1d[s][0], (plan->flags&FFT5D_BACKWARD) ? GMX_FFT_COMPLEX_TO_REAL : GMX_FFT_REAL_TO_COMPLEX, lin+tstart, lout+tstart);
    }
    else
    {
        gmx_fft_many_1d(     p1d[s][0], (plan->flags&FFT5D_BACKWARD) ? GMX_FFT_BACKWARD : GMX_FFT_FORWARD,               lin+tstart, lout+tstart);
    }
    /* ------------ END FFT ---------*/
