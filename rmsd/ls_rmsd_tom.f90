...
subroutine rmsd_dftb(n, coord1, coord2, U, x_center, y_center, error)
!-----------------------------------------------------------------------
...
!  If option=1... NO, DO IT ALWAYS
!   then the rotation matrix U and the centers of coord are returned.
!-----------------------------------------------------------------------
  ...
  call dstmev_dftb(S, lambda, q)

  ! convert quaternion q to rotation matrix U
  call rotation_matrix_dftb(q, U)

  ! RMS Deviation
  error = sqrt(max(0.0d0,((x_norm+y_norm)-2.0d0*lambda))/dble(n))

end subroutine rmsd_dftb
!-----------------------------------------------------------------------
subroutine rotation_matrix_dftb(q, U)
!-----------------------------------------------------------------------
...
end subroutine rotation_matrix_dftb
!-----------------------------------------------------------------------
subroutine dstmev_dftb(A,lambda,evec)
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
  ...
  call svdcmp_dftb(4,T,4,4,SW,SV,rv1)
  !-----------------------------------------------------------------------
...
end subroutine dstmev_dftb
!-----------------------------------------------------------------------
subroutine givens4_dftb(S,T,V)
!-----------------------------------------------------------------------
  ...
  !-----------------------------------------------------------------------
end subroutine givens4_dftb
!-----------------------------------------------------------------------
SUBROUTINE svdcmp_dftb(mmax,a,m,n,w,v,rv1)
!-----------------------------------------------------------------------
  integer :: mmax
  INTEGER :: m,n
...
!-----------------------------------------------------------------------
END subroutine svdcmp_dftb
!-----------------------------------------------------------------------
FUNCTION pythag_dftb(a,b)
!-----------------------------------------------------------------------

  real(dp) :: pythag_dftb, a, b
  real(dp) :: absa,absb

  absa=abs(a)
  absb=abs(b)
  if(absa.gt.absb)then
     pythag_dftb=absa*dsqrt(1.0d0+(absb/absa)**2)
  else
     if(absb.eq.0.0d0)then
        pythag_dftb=0.0d0
     else
        pythag_dftb=absb*dsqrt(1.0d0+(absa/absb)**2)
     endif
  endif

END function pythag_dftb
!-----------------------------------------------------------------------
