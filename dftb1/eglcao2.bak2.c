      ...
      for (jj = 0; jj < ct->site[j].homos; jj++) {
        ...
        if (counter % ct_mpi_size == ct_mpi_rank){
          ...
          if (ct_mpi_rank != 0) { 
            ...
            //printf("matrix element no. %d (%lf) sent from rank %d\n", counter,dftb2.THamil[ifo][jfo], ct_mpi_rank);
          ...
          //printf("matrix element no. %d (%lf) recieved from rank %d\n", counter, dftb2.THamil[ifo][jfo] ,counter % ct_mpi_size);
